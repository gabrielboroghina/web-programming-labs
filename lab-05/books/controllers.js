...
router.get('/', authorizeAndExtractToken, authorizeRoles('user', 'admin'), async (req, res, next) => {
    const data = await booksDbBridge.getAll();
    res.status(200).send(data);
});

router.get('/:id', authorizeAndExtractToken, authorizeRoles('user', 'admin'), async (req, res, next) => {
